...
public class GoogleTextToSpeech {
	 ...
	 */
	public GoogleTextToSpeech(Language language) {
	 ...
	 * language autoDetection
	 */
	public GoogleTextToSpeech(String languageCode) {
	...
	public InputStream getMP3Data(String synthText) throws Exception {
		...
		if (synthText.length() > 100) {
			List<String> fragments = parseString(synthText);// parses String if
															// too long
			String tmp = getLanguageCode();
			setLanguageCode(languageCode);// Keeps it from autodetecting each
											// fragment.
			InputStream out = getMP3Data(fragments);
			setLanguageCode(tmp);// Reverts it to it's previous Language such as
									// auto.
			return out;
		}
		...
		URL url = new URL(GOOGLE_SYNTHESISER_URL + languageCode + "&q="
				+ encoded); // create url
		...
		urlConn.addRequestProperty("User-Agent",
				"Mozilla/5.0 (Windows NT 6.1; WOW64; rv:2.0) Gecko/20100101 Firefox/4.0"); // Adding
																							// header
																							// for
																							// user
																							// agent
	...
	public InputStream getMP3Data(List<String> synthText) throws Exception {
		...
		for (String part : synthText) {
			complete = new java.io.SequenceInputStream(complete,
					getMP3Data(part));// Concatenate with new MP3 Data
		}
		return complete;
	}
	...
	public String detectLanguage(String text) throws Exception {
		if (text.length() > 99) {// Google will not compute more than 99
									// characters
			int lastWord = findLastWord(text);
			if (lastWord < 0) {
				text = text.substring(0, 99);// Fix for languages without
												// spaces.
			} else {
				text = text.substring(0, lastWord);// We don't need the whole
													// text to determine
													// language
			}
		}
		...
		if (!isLanguageSupported(rawData))
			return null;// Comment this if statement out if you want to use this
						// code for rare languages like Maori.
	 ...
	 * 
	 * @param urlConn
	 *            The Open URLConnection that you want to generate a String from
	 * @return The generated String
	 * @throws Exception
	 *             if it cannot complete the request
	 */
	private String urlToText(URLConnection urlConn) throws Exception {
		Reader r = new java.io.InputStreamReader(urlConn.getInputStream());// Gets
																			// Data
																			// Converts
																			// to
																			// string
	 ...
	 * @return The language parsed from the rawData or null if Google cannot
	 *         determine it.
	 */
	private String parseRawData(String rawData) {
		for (int i = 0; i + 5 < rawData.length(); i++) {
			boolean dashDetected = rawData.charAt(i + 4) == '-';// Sometimes
																// Google will
																// detect the
																// region too.
			if (rawData.charAt(i) == ','
				...
				if (dashDetected) {// If region is detected parses the whole
									// string!
					int lastQuote = rawData.substring(i + 2).indexOf('"');// Where
																			// the
																			// region
																			// ends
					if (lastQuote > 0)
					...
					if (containsLettersOnly(possible)) {// Required due to
														// Google's inconsistent
														// formatting.
	 ...
	 * 
	 * @param rawData
	 *            Checks if a language is supported based off of rawData
