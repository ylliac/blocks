...
public class Microphone {
	 ...
	 *         CLOSED is returned if the Thread is not doing anything/not
	 *         capturing audio
	 */
	...
	private void initTargetDataLine() {
		DataLine.Info dataLineInfo = new DataLine.Info(TargetDataLine.class,
				getAudioFormat());
		try {
			setTargetDataLine((TargetDataLine) AudioSystem
	 ...
	 * 
	 * @param audioFile
	 *            The fully path (String) to a file you want to save the audio
	 *            in
	 * @throws Exception
	 *             Throws an exception if something went wrong
	 */
	...
	public void open() {
		...
		if (!getTargetDataLine().isOpen() && !getTargetDataLine().isRunning()
				&& !getTargetDataLine().isActive()) {
			try {
	...
	private class CaptureThread implements Runnable {
		...
		public void run() {
			try {
				...
				AudioSystem.write(new AudioInputStream(getTargetDataLine()),
						fileType, audioFile);
