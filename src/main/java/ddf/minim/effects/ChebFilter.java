...
public class ChebFilter extends IIRFilter
{
  ...
  public void setType(int t)
  {
    if ( t != LP && t != HP )
    {
      Sound.error("Invalid filter type, defaulting to low pass.");
      t = LP;
    }
    type = t;
    calcCoeff();
  ...
  public void setPoles(int p)
  {
    ...
    if (p % 2 != 0)
    {
      Sound.error("ChebFilter.setPoles: The number of poles must be even.");
      return;
    }
    if (p > 20)
    {
      Sound.error("ChebFilter.setPoles: The maximum number of poles is 20.");
    }
    poles = p;
    calcCoeff();
    initArrays();
