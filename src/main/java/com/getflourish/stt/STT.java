...
public class STT {
	 ...
	 *            instance of PApplet
	 */
	public STT(TranscribeListener listener) {
		this(listener, false);
	}
	 ...
	 *            instance of PApplet
	 * @param history
	 *            indicates whether or not recordings are stored in the data
	 *            folder
	 */
	public STT(TranscribeListener listener, boolean history) {
		this(listener, history, null);
	}

	public STT(TranscribeListener listener, boolean history, Sound _minim) {
		this.listener = listener;
		this.log = history;
		this.threads = new ArrayList<TranscriptionThread>();
		if (minim == null) {
			this.minim = new Sound(new JSMinim());
		} else {
			this.minim = _minim;
		}
		this.encoder = new FLAC_FileEncoder();
		// get a LineIn from Minim, default bit depth is 16
		in = minim.getLineIn(Sound.MONO, 1024, 8000, 16);
		this.recorder = minim.createRecorder(in, path + fileName + fileCount
				+ ".wav", true);
		disableAutoRecord();
		this.listen();
	}
	...
	private void dispatchTranscriptionEvent(String utterance, float confidence,
		...
		if (listener != null && !status.equals(lastStatus)) {
			listener.transcribe(utterance, confidence, s);
		}
	}
	...
	public void draw() {
		...
		for (int i = 0; i < threads.size(); i++) {
			...
			if (transcriptionThread.isAvailable()) {
				if (listener != null) {
					dispatchTranscriptionEvent(
							transcriptionThread.getUtterance(),
							transcriptionThread.getConfidence(),
	...
	private void listen() {
		...
		if (listener == null) {
			System.err
					.println("STT info: use a TranscribeListener to receive transcription events");
		}
	}
	...
	public void onSpeechFinish() {
		...
		encoder.encode(new File(path + fileName + fileCount + ".wav"),
				new File(flac));
