...
public abstract class FourierTransform
{
  ...
  protected void setComplex(float[] r, float[] i)
  {
    ...
    {
      Sound.error("FourierTransform.setComplex: the two arrays must be the same length as their member counterparts.");
    }
    else
    {
  ...
  public void linAverages(int numAvg)
  {
    ...
    {
      Sound.error("The number of averages for this transform can be at most "
          + spectrum.length / 2 + ".");
      return;
    }
    else
  ...
  public void logAverages(int minBandwidth, int bandsPerOctave)
  {
    ...
    Sound.debug("Number of octaves = " + octaves);
  ...
  public void window(int which)
  {
    if (which < 0 || which > 1)
    {
      Sound.error("Invalid window type.");
    }
    else
    {
      whichWindow = which;
  ...
  public void forward(float[] buffer, int startAt)
  {
    ...
    {
      Sound.error( "FourierTransform.forward: not enough samples in the buffer between " + 
